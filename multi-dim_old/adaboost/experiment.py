...
from adaboost_adjusted import AdjustedAdaBoost
...
control_experiment = 'AdaBoost'

method_to_obj = {'AdaBoost': AdaBoost(n_estimators = 5),
	'AdaBoost - Flipped Labels': AdaBoost(n_estimators = 5),
	'AdaBoost - Numerical Correction': AdjustedAdaBoost(n_estimators = 5), 
	'Test Time Correction': TestTimeCorrection(AdaBoost(n_estimators = 5))}
...
class Experiment:
	...
	def get_loss(self, experiment = control_experiment, params = {}, is_dp = False):
		...
		elif is_dp and experiment == 'AdaBoost - Flipped Labels':
			classifier.fit(self.df_dp_unfiltered.df[self.f_names], self.df_dp_unfiltered.df[self.l_name])
