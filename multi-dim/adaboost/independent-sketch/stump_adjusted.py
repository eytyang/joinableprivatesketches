...
class AdjustedStump(BaseEstimator):
	def __init__(self, label_halls = None, feat_label_halls = None, count_minimum = 1):
		self.minimum = count_minimum
		self.label_counts = {}
		self.feat_label_counts = {}
		self.label_halls = label_halls
		self.feat_label_halls = feat_label_halls
		self.stump = None
		self.choice = {}
	...
	def populate_counts(self, features, labels, full_labels):
		...
		for l_val in [0, 1]:
			self.label_counts[l_val] = max(dp_label_count[l_val] - self.label_halls[l_val], self.minimum)
