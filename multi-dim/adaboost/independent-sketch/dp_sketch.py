...
# Translates the sketch into noisy feature values or noisy membership score
...
class DP_Join:
	def __init__(self, eps_memb, eps_val, num_buckets = None):
	...
	def join(self, df_known, df_private):
		...
		# Create and decode the membership sketch. 
		...
		# Create and decode a BinarySketch for each feature
	...
	# Drops entries based on membership
	def drop_entries(self):
		self.df = self.df[self.df['membership'] >= 1]

	# Flips labels based on membership
